...
<div class="card shadow-sm">
    ...
    <div class="card-body row ms-0 me-0 bg-secondary">
        <div class="uim text-center mb-3 col-lg-2">
            <LinkedPfpIcon {ndk} npub={user.npub} cls="lg"/>
        </div>
        <div class="ps-4 col-lg-10">
            <h3 class="card-title mb-2"><span class="userName"><UserName {user} /></span></h3>
            {#if user.profile.nip05}
                <span class="badge bg-dark p-2">
                    {user.profile.nip05}
                    {#if user.profile.nip05valid}
            ...
            {/if}
            <UserWebsite profile={user.profile} />
            <div class="card-text mt-3">{user.profile.about}</div>
